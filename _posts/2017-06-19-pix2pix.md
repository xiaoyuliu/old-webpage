...
0. **Generator**
    ...
    For generator, the paper uses a "U-Net"-based architecture, which is similar with encoder-decoder network. There are some skips in U-Net between each layer $$i$$ and $$n-i$$, which are helpful to share lots of low0level information between the input and output.
